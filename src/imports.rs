...
use crate::memory::{
    read_region, read_u32, read_utf16_string, ref_to_u32, to_u32, write_region, VmResult,
};
use crate::types::{
    GetIdentityAction, PromiseResult, ReadContractDataAction, ReadShardedDataAction,
};
...
pub fn process_gas_info<B: Backend>(env: &Env<B>, used_gas: u64) -> VmResult<()> {
...
pub fn write_to_contract<B: Backend>(env: &Env<B>, input: &[u8]) -> VmResult<u32> {
...
pub fn event<B: Backend>(env: &Env<B>, event_name: u32, args: u32) -> VmResult<()> {
    ...
    let args = if args > 0 {
        read_region(&env.memory(), args, MAX_ARGS_SIZE)?
    } else {
        vec![]
    };
    set_left_gas_to_backend(env)?;
...
pub fn create_call_function_promise<B: Backend>(
    env: &Env<B>,
    addr: u32,
    method: u32,
    args: u32,
    ...
    let amount_value = if amount > 0 {
        read_region(&env.memory(), amount, MAX_IDNA_SIZE)?
    } else {
        vec![]
    };
    ...
    let idx_res =
        env.create_function_call_promise(to, method, args, amount_value, gas_limit as u64);
    let idx = idx_res.0?;
...
pub fn create_deploy_contract_promise<B: Backend>(
    env: &Env<B>,
    code: u32,
    args: u32,
    nonce: u32,
    ...
    let nonce = if nonce > 0 {
        read_region(&env.memory(), nonce, MAX_STRING_SIZE)?
    } else {
        vec![]
    };
    let amount_value = if amount > 0 {
    ...
    deduct_balance_if_needed(env, &amount_value)?;
    let idx_res =
        env.create_deploy_contract_promise(code, args, nonce, amount_value, gas_limit as u64);
    let idx = idx_res.0?;
    process_gas_info(env, gas_limit as u64)?;
    process_gas_info(env, idx_res.1)?;
    Ok(idx)
...
pub fn contract_addr_by_hash<B: Backend>(
    env: &Env<B>,
    hash: u32,
    args: u32,
    nonce: u32,
...
pub fn create_read_contract_data_promise<B: Backend>(
    env: &Env<B>,
    addr: u32,
    key: u32,
    gas_limit: u32,
    ...
    let idx_res = env.create_read_sharded_data_promise(
        to,
...
pub fn create_get_identity_promise<B: Backend>(
    env: &Env<B>,
    addr: u32,
    gas_limit: u32,
) -> VmResult<u32> {
    ...
    let idx_res = env.create_read_sharded_data_promise(
        to.clone(),
        ReadShardedDataAction::GetIdentity(GetIdentityAction {
            addr: to,
            gas_limit: gas_limit as u64,
        }),
    );
